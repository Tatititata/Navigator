...
git clone git@github.com:Tatititata/Navigator.git
cd Navigator
```
```bash
...
Исполняемый файл 'graph' будет создан в директории Navigator
...
Next n lines: adjacency matrix (n x n) with integer weights \
0 indicates no edge (or loop with weight 0) \
Первая строка: количество вершин (n) \
Следующие n строк: матрица смежности (n x n) с целочисленными весами \
0 означает отсутствие ребра (или петля с весом 0)
...
Ant Colony Algorithm (main implementation) \
Муравьиный алгоритм (основная реализация)

Closest Neibourgh (additional algorithm) \
Метод ближайшего соседа (дополнительный алгоритм)

Simulated Annealing (additional algorithm) \
Имитация отжига (дополнительный алгоритм)
